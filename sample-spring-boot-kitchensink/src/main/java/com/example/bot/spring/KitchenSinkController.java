...
//Create image file here
        try{
            Robot r = new Robot();
            String path = "C:/gitdir/line-bot-sdk-java/sample-spring-boot-kitchensink/src/main/resources/img/"+filename+".jpg"; 
            
            Rectangle capture =  new Rectangle(0,0,800,600); 
...
//		return new ReplyMessage(event.getReplyToken(), Arrays.asList(imageMessage, textMessage));
    ...
    private static final String BT_2 = "btall";
    private static final String SC_MAIN = "sc";    
    private String runfunct(String command,String replyToken) throws Exception {
        Robot r = new Robot();
        r.delay(100);
        switch (command) {
            case INVITE_MAIN:
            ...
            case RIFT_MAIN:
                ...
                break;
            case SC_MAIN:
                String generatedString = makefilename();
                screenshot(generatedString);
                URI imageUrl = createUri("/img/"+generatedString+".jpg");
                this.reply(
                    replyToken,
                    Arrays.asList(new TextMessage("Getting status"),
                    new TextMessage(
                    "current status , last command time "),
                    new ImageMessage(imageUrl,
                    imageUrl ))
                );
                break;
        }
...
*/
        ...
        switch (text) {
            case SC_MAIN: {
                runfunct(SC_MAIN,replyToken);
                break;
            }
            case INVITE_MAIN: {
                runfunct(INVITE_MAIN,replyToken);
                break;
            }
            case RESTART_MAIN: {
                runfunct(RESTART_MAIN,replyToken);
                break;
            }
            case RIFT_MAIN: {
                runfunct(RIFT_MAIN,replyToken);
                break;
            }
            case "profile": {
            ...
            case "x": {
                URI imageUrl = createUri("/static/buttons/1041.jpg");
